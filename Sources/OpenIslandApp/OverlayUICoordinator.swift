...
final class OverlayUICoordinator {
    ...
    private func updateNotificationAutoCollapse() {
        ...
        notificationAutoCollapseTask = Task { @MainActor [weak self] in
            ...
            // 紫微：通知卡到点后，若还有存活会话，回落到会话列表保持展开
            // （像 Vibe Island 一样常驻显示当前对话），而不是收成空胶囊。
            if let appModel = self.appModel,
               appModel.keepIslandExpandedWhileActive,
               !appModel.surfacedSessions.isEmpty {
                self.notchOpen(reason: .click, surface: .sessionList())
            } else {
                self.notchClose()
            }
        }
