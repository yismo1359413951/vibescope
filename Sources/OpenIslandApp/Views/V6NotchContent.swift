...
struct V6ClosedPill: View {
    ...
    // 帝星按会话状态变色（对应 Vibe Island 头像绿/橙的状态语义）：
    // idle 淡紫静息（不急闪）、running 亮马卡龙紫（活跃闪烁）、waiting 淡金（提示注意）。
    private var starColor: Color {
        switch mode {
        case .idle:    return Color(red: 0xB4 / 255.0, green: 0x9E / 255.0, blue: 0xE0 / 255.0).opacity(0.75)
        case .running: return Color(red: 0xC2 / 255.0, green: 0xAA / 255.0, blue: 0xEE / 255.0)
        case .waiting: return Color(red: 0xE6 / 255.0, green: 0xCF / 255.0, blue: 0x94 / 255.0)
        }
    }

    private var starLively: Bool {
        mode != .idle
    }

    // 工作/等待态在星旁显示跳动均衡器条（对应 Vibe Island 的活动条），
    // 静息态不显示，保持"一颗静星"的克制。
    private static let barsWidth: CGFloat = 20
    private static let barsGap: CGFloat = 3
    private var showsActivityBars: Bool { mode != .idle }
    ...
    private var externalBody: some View {
        ...
        let barsBlock = (showsActivityBars ? Self.barsGap + Self.barsWidth : 0)
        let labelBlock = (label == nil ? 0 : 6 + labelW)
        let rightBlock = (rightSlot == nil ? 0 : Self.innerGap + rightW)
        let intrinsic = pad * 2 + glyphW + barsBlock + labelBlock + rightBlock
        ...
        return ZStack {
            ...
            HStack(spacing: 0) {
                PixelStarView(size: 24, lively: starLively, color: starColor)
                    .frame(width: glyphW, height: 24)

                if showsActivityBars {
                    UnifiedBars(mode: mode, size: Self.barsWidth, tint: starColor)
                        .frame(width: Self.barsWidth, height: 24)
                        .padding(.leading, Self.barsGap)
