...
struct IslandPanelView: View {
        ...
        Button {
            ...
            HStack(spacing: 8) {
                Image(systemName: "sparkles")
                    ...
                    .foregroundStyle(V6Palette.accent)
            ...
            .background(
                RoundedRectangle(cornerRadius: 10, style: .continuous)
                    .fill(V6Palette.accent.opacity(0.14))
                    .overlay(
                        RoundedRectangle(cornerRadius: 10, style: .continuous)
                            .stroke(V6Palette.accent.opacity(0.35), lineWidth: 0.5)
                    )
            )
        }
        .buttonStyle(.plain)
    ...
    /// 无活跃会话时的静候态。对齐 Vibe Island —— 不甩"没有打开的终端会话"
    /// 那种冷冰冰的空状态，而是克制地显示品牌 + 一句轻提示。
    private var emptyState: some View {
        VStack(spacing: 8) {
            Spacer()
            Image(systemName: "sparkles")
                .font(.system(size: 17, weight: .light))
                .foregroundStyle(V6Palette.accent.opacity(0.75))
        ...
        VStack(spacing: 0) {
            ...
            if isNotificationMode, let session = model.activeIslandCardSession {
                IslandSessionRow(
                    ...
                    autoApprovePaused: model.isAutoApprovePaused(session.id),
                    onToggleAutoApprove: { model.toggleAutoApprovePause(session.id) }
                )
...
private struct IslandSessionRow: View {
    ...
    let onJump: () -> Void
    var onDismiss: (() -> Void)?
    /// 该会话是否已暂停自动批准（决定 badge 文案/样式）。
    var autoApprovePaused: Bool = false
    /// 点按 badge 切换该会话的自动批准暂停状态。
    var onToggleAutoApprove: (() -> Void)?
    ...
    private var showsAutoApproveBadge: Bool {
        guard AutoApproveStore().rules.enabled else { return false }
        switch session.tool {
    ...
    private var autoApproveBadge: some View {
        // 放行中=薄荷绿(在自动批准)；已暂停=灰(改手动)。点按切换。
        let activeTint = Color(red: 0x6F / 255.0, green: 0xB8 / 255.0, blue: 0xA0 / 255.0)
        let pausedTint = Color.white.opacity(0.5)
        let tint = autoApprovePaused ? pausedTint : activeTint
        let title = autoApprovePaused ? "已暂停" : "自动批准 ✓"
        return Button {
            onToggleAutoApprove?()
        } label: {
            Text(title)
    ...
    private var shouldShowEmbeddedDetailBody: Bool {
        ...
        // 紫微：运行态不再显示命令/文件路径/Thinking 框，保持列表干净清爽
        // （像 Vibe Island，行里只留 头像 + 标题 + 你：那句话 + 徽章）。
        return false
    }
    ...
    private var commandPreviewText: String {
        if let preview = cleanedCommandPreview {
            return "$ \(preview)"
        }
        return session.permissionRequest?.summary.trimmedForNotificationCard ?? session.summary.trimmedForNotificationCard
    }

    /// 清洗过的命令预览：Bash 的 JSON 解包成纯命令，避免批准框 / 详情甩出原始 `{"command":…}`。
    private var cleanedCommandPreview: String? {
        guard let raw = session.currentCommandPreviewText?
            .trimmingCharacters(in: .whitespacesAndNewlines), !raw.isEmpty else {
            return nil
        }
        let clean = AgentSession.surfacePreview(forTool: session.currentToolName ?? "", raw: raw)
    ...
    private func statusIndicator(for presence: IslandSessionPresence) -> some View {
        ...
        case .animatedDot:
            PixelAvatarView(
                seed: session.id,
                size: 18,
                pulsing: presence == .running || isActionable
            )
            .opacity(presence == .inactive ? 0.5 : 1)
...
extension MarkdownUI.Theme {
    @MainActor static let completionCard = Theme()
        ...
        // 折叠卡片很小：标题字号 = 折叠条标签(11.5pt)，全部一样大，只用加粗区分层级，
        // 不让 Claude 消息里的 #/## 标题在小卡里显得"大"。
