...
final class AppModel {
    ...
    // MARK: - 自动批准·按会话暂停（UI 可观察镜像，写穿到 AutoApproveStore 供 BridgeServer 读取）

    private let autoApproveStore = AutoApproveStore()
    /// 已"暂停自动批准"的会话 ID（@Observable → badge 实时刷新）。
    /// 本次启动内为权威；BridgeServer 经 UserDefaults 读到同一份。
    private(set) var pausedAutoApproveSessionIDs: Set<String> = []

    func isAutoApprovePaused(_ sessionID: String) -> Bool {
        pausedAutoApproveSessionIDs.contains(sessionID)
    }

    /// 切换某会话的自动批准暂停状态：更新可观察镜像 + 写穿到 Store。
    func toggleAutoApprovePause(_ sessionID: String) {
        let nowPaused = !pausedAutoApproveSessionIDs.contains(sessionID)
        if nowPaused {
            pausedAutoApproveSessionIDs.insert(sessionID)
        } else {
            pausedAutoApproveSessionIDs.remove(sessionID)
        }
        autoApproveStore.setAutoApprovePaused(nowPaused, forSession: sessionID)
    }
    ...
    /// 折叠胶囊标签截断，保持"一小条"（像 Vibe Island）。
    private static func truncatedPillLabel(_ text: String, max: Int = 22) -> String {
    ...
    func islandClosedLabel() -> String? {
        ...
        // 静息态（无活跃会话）：显示品牌名「紫微」，避免折叠条塌成"孤星"。
        // 对应 Vibe Island 静息时显示 "Vibe Island" —— 永远是个完整的小条。
        guard let session = islandClosedSpotlight else {
            return Self.idleBrandLabel
        }
        ...
        case .agentAction:
            // 紫微：折叠胶囊像 Vibe Island 一样显示"工具 + 当前命令"，但**截断成小短条**
            // （如 "Bash cd ~/Desk…"），不让长命令把胶囊撑长。
            if let activity = session.spotlightRunningActivityText, !activity.isEmpty {
                return Self.truncatedPillLabel(activity)
            }
            if let action = session.displayCurrentToolName, !action.isEmpty {
                return Self.truncatedPillLabel("\(session.tool.displayName) · \(action)")
            }
            // 会话在跑但没有活跃工具（Claude 正在思考/回话）→ 优先显示"用户当前
            // 在让 Claude 做什么"（最近诉求，截断），比干巴巴"工作中…"更有信息量、
            // 更贴近 Vibe Island 的手感；取不到再退回"工作中…"。
            if session.phase == .running {
                if let prompt = session.latestUserPromptText?
                    .trimmingCharacters(in: .whitespacesAndNewlines), !prompt.isEmpty {
                    return Self.truncatedPillLabel(prompt)
                }
                return "\(session.tool.displayName) · 工作中…"
            }
            return session.tool.displayName
        }
    }

    /// 静息态折叠条显示的品牌名（无活跃会话时），避免胶囊塌成"孤星"。
    private static let idleBrandLabel = "紫微"
    ...
    func applyTrackedEvent(
        ...
        maintainExpandedSessionListIfActive()
    }

    /// 紫微：是否在有活动会话时让岛保持展开（默认开）。以后可在设置里加开关。
    var keepIslandExpandedWhileActive: Bool {
        get { UserDefaults.standard.object(forKey: "keepIslandExpandedWhileActive") as? Bool ?? false }
        set { UserDefaults.standard.set(newValue, forKey: "keepIslandExpandedWhileActive") }
    }

    /// 紫微：编程时保持岛展开成会话列表（而非缩成胶囊）。
    /// 只在岛当前折叠、且存在活动会话时自动展开；不打断已弹开的审批/通知卡
    /// （那时 notchStatus != .closed，守卫直接返回）。因为不是用户悬停打开的，
    /// 鼠标离开不会触发自动折叠，所以列表会持续显示，直到无活动会话或用户手动关。
    private func maintainExpandedSessionListIfActive() {
        guard keepIslandExpandedWhileActive else { return }
        guard notchStatus == .closed else { return }
        // 只要有可见（存活）会话就保持岛展开成列表，空闲时也不收
        // （像 Vibe Island 那样常驻显示当前对话）。
        guard !surfacedSessions.isEmpty else { return }
