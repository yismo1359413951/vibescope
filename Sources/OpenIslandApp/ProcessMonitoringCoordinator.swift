...
final class ProcessMonitoringCoordinator {
    ...
    func sessionIDsWithAliveProcesses(
        ...
        // Claude 桌面 App 会话：跑在 Claude.app 里的 Claude Code 没有可匹配的
        // `claude` CLI 子进程，上面 Pass 1–3 永远扫不到它。没有保活的话，会被
        // SessionState.markProcessLiveness 里"连续两次没扫到就标 ended"的兜底
        // 误杀，导致会话明明还活着却从岛上消失（折叠条偶尔闪一下命令、点开却
        // 是空的）。照 Cursor / Codex.app 的做法：Claude.app 在跑时，把这类没有
        // 真实终端的桌面会话保活，已完成的按 staleness 超时自然淡出。
        if Self.isClaudeDesktopAppRunning() {
            for session in trackedClaudeSessions where !claimedSessionIDs.contains(session.id) {
                guard !session.isSessionEnded, session.isClaudeDesktopSession else { continue }
    ...
    /// Check whether the Claude desktop app is currently running.  Uses the
    /// same direct `runningApplications` scan as ``isCodexDesktopAppRunning``
    /// to avoid the intermittent empty-array behavior of the bundle-ID query.
