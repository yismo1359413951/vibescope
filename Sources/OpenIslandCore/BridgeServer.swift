...
public final class BridgeServer: @unchecked Sendable {
    ...
    /// Supplies the current auto-approve rules at decision time. Injected as a
    /// closure (rather than a static value) so the user can edit rules in
    /// settings and have changes take effect without restarting the bridge.
    /// Tests inject a fixed rule set.
    private let autoApproveRulesProvider: @Sendable () -> AutoApproveRules

    /// 查询某会话是否已"暂停自动批准"（用户在岛上点了 badge）。暂停时把引擎的
    /// allow 改判为 ask（手动确认）；deny 不受影响（安全拦截不可被会话开关绕过）。
    private let autoApprovePausedProvider: @Sendable (String) -> Bool

    public init(
        socketURL: URL = BridgeSocketLocation.defaultURL,
        autoApproveRulesProvider: @escaping @Sendable () -> AutoApproveRules = { AutoApproveStore().rules },
        autoApprovePausedProvider: @escaping @Sendable (String) -> Bool = { AutoApproveStore().isAutoApprovePaused(forSession: $0) }
    ) {
        self.socketURL = socketURL
        self.autoApproveRulesProvider = autoApproveRulesProvider
        self.autoApprovePausedProvider = autoApprovePausedProvider
        queue.setSpecific(key: queueKey, value: ())
    }

    /// 应用按会话暂停：若该会话已暂停且决定为 allow，则改判为 ask（手动）。
    /// deny / 已经是 ask 的不变。
    private func effectiveBehavior(
        _ behavior: AutoApproveBehavior,
        sessionID: String
    ) -> AutoApproveBehavior {
        guard behavior == .allow, autoApprovePausedProvider(sessionID) else {
            return behavior
        }
        return .ask
    }
    ...
    private func handleCodexHook(_ payload: CodexHookPayload, from clientID: UUID) {
        ...
        case .preToolUse:
            ...
            let preToolDecision = ApprovalRuleEngine(rules: autoApproveRulesProvider())
                .evaluate(codexApprovalContext(payload))
            ...
            switch effectiveBehavior(preToolDecision.behavior, sessionID: payload.sessionID) {
            case .ask:
                emit(
            ...
            case .allow:
                resolvePendingApproval(sessionID: payload.sessionID, resolution: .allowOnce())
            case .deny:
                resolvePendingApproval(
                    sessionID: payload.sessionID,
                    resolution: .deny(
                        message: "Auto-denied by Open Island rule (\(preToolDecision.reason)).",
                        interrupt: false
                    )
                )
            }
        ...
        case .permissionRequest:
            ...
            let permissionDecision = ApprovalRuleEngine(rules: autoApproveRulesProvider())
                .evaluate(codexApprovalContext(payload))
            ...
            switch effectiveBehavior(permissionDecision.behavior, sessionID: payload.sessionID) {
            case .ask:
                emit(
                    .permissionRequested(
                        PermissionRequested(
            ...
            case .deny:
                resolvePendingApproval(
                    sessionID: payload.sessionID,
                    resolution: .deny(
                        message: "Auto-denied by Open Island rule (\(permissionDecision.reason)).",
                        interrupt: false
                    )
                )
            }
    ...
    private func handleClaudeHook(_ payload: ClaudeHookPayload, from clientID: UUID) {
        ...
        case .permissionRequest:
            ...
            if let prompt = payload.questionPrompt {
                ...
                // Consult the auto-approve engine before bothering the user.
                // 先过自动批准规则引擎，命中规则就不打扰用户。
                let decision = ApprovalRuleEngine(rules: autoApproveRulesProvider())
                    .evaluate(claudeApprovalContext(payload))

                // Every branch registers the pending interaction first so the
                // shared resolve path (emit + response) handles both manual and
                // automatic outcomes identically.
                pendingClaudeInteractions[payload.sessionID] = PendingClaudeInteraction(
                    clientID: clientID,
                    kind: .permission(payload)
                )

                switch effectiveBehavior(decision.behavior, sessionID: payload.sessionID) {
                case .ask:
                ...
                case .deny:
                    resolvePendingClaudeInteraction(
                        sessionID: payload.sessionID,
                        resolution: .deny(
                            message: "Auto-denied by Open Island rule (\(decision.reason)).",
                            interrupt: false
                        )
                    )
                }
    ...
    /// Flattens a Claude permission payload into the agent-agnostic
    /// ``ApprovalContext`` the rule engine understands: the tool name, the shell
    /// command (for Bash-like tools), any file paths it would touch, and the cwd.
    private func claudeApprovalContext(_ payload: ClaudeHookPayload) -> ApprovalContext {
        var command: String?
        var filePaths: [String] = []
        if case let .object(object) = payload.toolInput {
            if case let .string(value)? = object["command"] {
                command = value
            }
    ...
    /// Flattens a Codex permission payload into an ``ApprovalContext``. Codex
    /// preToolUse requests are shell commands, so the command text is the key
    /// signal the rule engine matches on.
    private func codexApprovalContext(_ payload: CodexHookPayload) -> ApprovalContext {
        ApprovalContext(
            agent: .codex,
