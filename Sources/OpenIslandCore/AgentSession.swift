...
public extension AgentSession {
    ...
    /// 后台"观察者"会话：例如 claude-mem 插件用 headless `claude` 在
    /// `~/.claude-mem/observer-sessions` 下反复拉起的记忆生成会话。这些不是
    /// 用户的交互式编码会话，不该在岛上露出，否则会把清单刷屏。
    var isBackgroundObserverSession: Bool {
        if let cwd = jumpTarget?.workingDirectory, cwd.contains("/.claude-mem/") {
            return true
        }
        // 兜底：cwd 还没解析出来时，按 claude-mem 观察者目录的工作区名识别。
        return jumpTarget?.workspaceName == "observer-sessions"
    }

    var isVisibleInIsland: Bool {
        if isBackgroundObserverSession { return false }
